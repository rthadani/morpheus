...
(def ^:private spec-rubric
  (str/join "\n"
    ["## You are reviewing a Wiggum implementation spec, not application code."
     ...
     "     :judge-mode :review-threshold :fallback-model :fallback-delay-ms"
     ...
     "     :temperature, :max-tokens (morpheus does not thread these to the"
     "     claude CLI — pure dead config)."
     ""
     ...
     "   * Non-Anthropic providers have :provider, plus :base-url and"
     "     :auth-token-env where the morpheus build-cmd+env requires them"
     "     (kimi/glm/openrouter/custom)."
     "   * Provider keyword (:ollama :kimi :glm :openrouter :anthropic) stripped"
     "     from the slug and the objective title."
     ""
