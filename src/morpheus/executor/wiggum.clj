     ...
     (str "> **Working directory**: you are already in the project root.\n"
          ...
          (if research?
            ...
            (str "> **Output style — save tokens.** The user's quota covers every comment, docstring, and chat line you emit. Documentation is added out-of-band after the run finishes, so do not produce any here.\n"
                 ...
                 "> - Avoid **orphan** global state. The test is not \"atom vs no atom\" — it is \"who owns this state's lifecycle?\". State held inside a component/system map (integrant, mount), a connection pool, a bounded cache with eviction, channel mults, or memoization is fine because those have owners that dispose of them. Orphaned module-level state with no lifecycle owner is never fine. Language-specific forms to reject:\n"
                 ">   - **Clojure**: top-level `(defonce app-state (atom {}))`, atoms used as a stand-in for a database, `(def paused (atom false))` toggling behavior\n"
                 ">   - **TypeScript / JavaScript**: `let state = {}` or `export const store = new Map()` at module scope, module-load singletons like `const client = new DB()` that are never closed\n"
                 ">   - **Python**: module-level dicts (`CACHE = {}`), class-level mutable defaults (`class Foo: items = []`), module globals used as implicit channels between functions\n"
                 ">   - **Go**: package-level `var mu sync.Mutex` paired with mutable package vars, `var db *sql.DB` at package scope\n"
                 ">   - **Java / Kotlin**: `static` mutable fields on a class, singleton objects with settable state\n"
                 ">   - **Rust**: `lazy_static!` or `once_cell` holding a `Mutex<Vec<_>>` or similar mutable container at crate scope\n"
                 ">   The fix in every language is the same: pass state through function arguments or scope it to a component with a start/stop lifecycle. Do not introduce module-level mutable state because \"state has to live somewhere\".\n"
                 ...
                 "> - **Concurrency**: never use `sleep`/`time.Sleep`/`Thread.sleep` to coordinate between concurrent units — use channels, semaphores, condition variables, or promises. Never spawn goroutines/threads/promises in an unbounded loop; apply backpressure with a worker pool or bounded channel. Do not hold a lock across I/O or an external call — acquire, mutate local state, release; do I/O outside the critical section.\n"
                 "> - **Resource lifecycle**: close what you open. Connections, file handles, HTTP clients, and pools must be released in `defer`/`with-open`/`try-with-resources`/`using` opened at the same scope as acquisition. Do not rely on GC.\n"
                 "> - **Test hygiene**: each test is self-contained — it sets up and tears down its own fixtures and shares no mutable state with other tests. Seed or stub all non-deterministic inputs (`time.now()`, `random()`, UUIDs) so failures are reproducible.\n"
                 "> - **Secrets**: never hardcode credentials, API keys, or tokens. Read from environment variables; fail fast with a descriptive error if the variable is absent.\n\n"))
          "# Objective\n\n"
          (:objective packet)
          (when (seq plan)
            (str "\n\n## This iteration\n\n"
